...
class NOMADAdapter(BaseAdapter):
    ...
    def fetch(self, doi: str) -> AdapterResult:
        ...
        try:
            ...
            try:
                # Use sub-path relative to the new base_url
                    ...
                    for ref in refs:
                        ...
                        for struct in structs:
                            ...
                            links.append(DatasetLink(
                                source_doi=doi,
                                dataset_url=f"https://nomad-lab.eu/prod/v1/optimade/structures/{struct_id}",
            ...
            if not links:
                params = {"q": f'references.doi:"{normalized_input}"'}
                response = self.session.get("entries", params=params)
                if response and response.status_code == 200:
